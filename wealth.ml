...
let ether_own_add amt = 
  if (amt > 0.) then 0.0 else raise (InvalidEtherAmount "cannot buy a non-positive amount of ether")
(** [ether_own_sub amt_ether] updates the log file to reflect recent
    Ether sell*)
let ether_own_sub amt = 
  if (amt > 0.) then 0.0 else raise (InvalidEtherAmount "cannot sell a non-positive amount of ether")
